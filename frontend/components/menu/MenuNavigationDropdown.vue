  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <ListboxOption
            v-for="menuEntry in (sidebarType === SidebarType.ORGANIZATION_PAGE ? menuEntryState.organizationEntry.value : menuEntryState.eventEntry.value)"
              ...
              <li
                ...
                'text-light-special-text dark:text-dark-special-text fill-light-special-text dark:fill-dark-special-text':
                  !active && !menuEntry.active,
                      ...
                      >
                <Icon :name="menuEntry.iconURL" class="w-5 h-5 mr-4 align-middle" aria-hidden="true" />
                <span class="block truncate" :class="{ 'font-medium': selected, 'font-normal': !selected }">{{ $t(menuEntry.label) }}</span>
              </li>
            </NuxtLink>
          </ListboxOption>
...
watchEffect(() => {
  selectedMenuItem.value = useRouter().currentRoute.value.fullPath.includes("organizations") ? menuEntryState.organizationEntry.value.find(e => e.selected) :
