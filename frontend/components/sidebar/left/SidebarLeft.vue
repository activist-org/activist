<template>
  <aside @mouseover="collapseSidebar(false)"
    ...
    <div ref="content"
      ...
      <!-- Add PageFilter for specific sidebarType -->
      <PageFilter v-if="sidebarType === SidebarType.ORGANIZATION_FILTER || sidebarType === SidebarType.EVENT_FILTER"
                  :showSearch="true"
                  defaultTitle="Filter by">
        <!-- Slot for additional custom controls -->
        <template #controls>
          <!-- Your custom filter controls for Organization or Event -->
        </template>
      </PageFilter>

      <SidebarLeftIndex v-if="
...
<script setup lang="ts">
  ...
  // Reactive state for filtered data (organizations or events)
  const filteredData = ref([]);

  // Capture emitted events from PageFilter
  function onLocationChange(newLocation) {
    locationFilter.value = newLocation;
    fetchFilteredData();
  }

  function onTopicsChange(newTopics) {
    topicFilters.value = newTopics;
    fetchFilteredData();
  }
  ...
  async function fetchFilteredData() {
    try {
      ...
      // Add location filter if it's set
      if (locationFilter.value) {
        queryParams.append('location', locationFilter.value);
      }

      // Add topics filter if any topics are selected
      if (topicFilters.value.length) {
        queryParams.append('topics', topicFilters.value.join(','));
      }

      // Construct the API URL
      const apiUrl = `/api/organizations?${queryParams.toString()}`;
