<template>
  <div class="relative flex w-full flex-col md:flex-row">
    <div class="flex w-full justify-center md:w-fit">
      <div
        class="w-fit border border-light-section-div bg-light-layer-0 dark:border-dark-section-div dark:bg-dark-layer-0"
        :class="{
          'rounded-lg': group.imageURL,
          'rounded-3xl': !group.imageURL,
        }"
      >
        <img
          v-if="group.imageURL"
          :class="{
            'h-[150px] w-[150px]': reduced,
            'h-[200px] w-[200px]': !reduced,
          }"
          :src="group.imageURL"
          :alt="
            $t('components.card-search-result-organization.img-alt-text') +
            ' ' +
            group.name
          "
        />
        <div
          v-else
          :class="{
            'h-[150px] w-[150px]': reduced,
            'h-[200px] w-[200px]': !reduced,
          }"
          class="flex items-center justify-center text-light-text dark:text-dark-text"
        >
          <Icon name="IconOrganization" class="h-[100%] w-[100%]" />
        </div>
      </div>
    </div>
    <div
      class="flex-col pt-3 md:grow md:pl-4 md:pt-0 lg:pl-6"
      :class="{
        'space-y-2': reduced,
        'space-y-3 md:space-y-4': !reduced,
      }"
    >
      <div class="flex flex-col justify-between md:flex-row">
        <div class="flex items-center justify-center space-x-2 md:space-x-4">
          <h2 class="responsive-h3 font-bold">
            {{ group.name }}
          </h2>
          <MenuSearchResult
            class="max-md:absolute max-md:right-0 max-md:top-0"
            :group="group"
          />
        </div>
        <div class="hidden items-center space-x-3 md:flex lg:space-x-5">
          <MetaTagLocation :location="group.location" />
        </div>
      </div>
      <div class="flex flex-col space-y-3 md:flex-row md:space-y-0">
        <div class="flex items-center justify-center space-x-4 md:hidden">
          <MetaTagLocation :location="group.location" />
        </div>
        <div
          class="flex justify-center space-x-3 md:justify-start lg:space-x-4"
        >
          <MetaTagOrganization :organizations="[group.organization.name]" />
          <MetaTagMembers
            :members="group.members"
            label="components._global.members"
          />
          <MetaTagSupporters
            :supporters="group.supporters"
            label="components.meta-tag.supporters_lower"
          />
        </div>
      </div>
      <div class="flex justify-center md:justify-start">
        <ShieldTopic v-if="!reduced" :topic="group.topic" />
      </div>
      <div class="flex justify-center md:justify-start">
        {{ group.description }}
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import type { Group } from "~/types/group";

defineProps<{
  group: Group;
  reduced?: boolean;
}>();
</script>
