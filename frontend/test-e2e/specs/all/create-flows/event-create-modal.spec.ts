...
test.describe("Event Create Modal", { tag: ["@desktop", "@all"] }, () => {
  // MARK: Modal opens
  ...
  test("full flow (online event) closes modal on submit", async ({
    ...
    await expect(modal.eventTypeForm).toBeVisible();
    await modal.locationTypeSection.getByRole("radio", { name: /online/i }).click();
  ...
  test("create another checkbox returns to step 1 after submit and keeps modal open", async ({
    ...
    await expect(modal.linkOnlineForm).toBeVisible();
    await modal.onlineLinkField.fill("https://example.com/create-another-test");
    await modal.getNextStepButton().click();
  ...
  test("full flow (physical event) closes modal on submit", async ({
    ...
    await expect(modal.eventTypeForm).toBeVisible();
    await modal.locationTypeSection.getByRole("radio", { name: /physical/i }).click();
    await modal.eventTypeSection.getByRole("radio", { name: /learn/i }).click();
    await selectFirstTopic(modal);
    await modal.getNextStepButton().click({ force: true });
    ...
    const germanyOption = modal.root.getByRole("option", { name: /Germany/i });
    ...
    const firstOptionRow = locationOptions.locator("div.style-menu-option").first();
    await firstOptionRow.waitFor({ state: "visible", timeout: 15000 }).catch(() => {});
    const hasLocationOptions = await firstOptionRow.isVisible().catch(() => false);
    ...
    await modal.root.locator("#event-location-submit").scrollIntoViewIfNeeded();
