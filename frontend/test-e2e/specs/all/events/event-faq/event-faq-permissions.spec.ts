...
test.describe(
  ...
  () => {
    ...
    test(
      "FAQ edit button is not visible on any card",
      async ({ page }, testInfo) => {
        logTestPath(testInfo);
        const { faqPage } = newEventPage(page);
        await withTestStep(
    ...
    test(
      "FAQ delete button is not visible on any card",
      async ({ page }, testInfo) => {
        logTestPath(testInfo);
        const { faqPage } = newEventPage(page);
        await withTestStep(
...
test.describe("Admin can manage event FAQs", { tag: ["@desktop"] }, () => {
  ...
  test(
    "FAQ edit button is visible on first FAQ card",
    async ({ page }, testInfo) => {
      logTestPath(testInfo);
      const { faqPage } = newEventPage(page);
      await withTestStep(
  ...
  test(
    "FAQ delete button is visible on first FAQ card",
    async ({ page }, testInfo) => {
      logTestPath(testInfo);
      const { faqPage } = newEventPage(page);
      await withTestStep(
