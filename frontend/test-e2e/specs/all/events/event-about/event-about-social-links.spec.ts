...
test.describe("Event About Page - Social Links", { tag: ["@desktop"] }, () => {
  ...
  test("User can manage social links (CREATE, UPDATE, DELETE)", async ({
    ...
    const submitButton = socialLinksModal.submitButton(socialLinksModal.modal);
    ...
    if (targetIndex === -1) {
      throw new Error("Could not find the social link we created for updating");
    }
