...
test.describe(
  ...
  () => {
    ...
    test("User can upload image (CREATE, UPDATE, DELETE)", async ({ page }) => {
      ...
      // Verify the number of image in the carousel matches the number of files in the modal.
      // It is 3 since the default number of images in the Image Carousel is 3 images.
      // TODO: Need to count the initial number of images in the Image Carousel instead
      await expect(
        organizationPage.aboutPage.getImageCarouselImages
      ).toHaveCount(3);
    });
