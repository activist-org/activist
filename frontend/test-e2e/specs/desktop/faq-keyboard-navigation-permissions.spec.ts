...
test.describe("Event FAQ Keyboard Permissions - Not Signed In", { tag: "@desktop" }, () => {
  ...
  test("not signed in: cannot focus FAQ cards (canEdit should be false)", async ({ page }) => {
    const eventPage = newEventPage(page);
    ...
    if (faqCardsVisible) {
      ...
      const focusedCards = await page.locator('[data-testid="faq-card"]:focus').count();
      expect(focusedCards).toBe(0);
    }
  });

  test("not signed in: no edit controls visible (canEdit should be false)", async ({ page }) => {
    const eventPage = newEventPage(page);
    const { faqPage } = eventPage;
    ...
    if (faqCardsVisible) {
      const dragHandles = await page.locator('[data-testid="faq-drag-handle"]').count();
      expect(dragHandles).toBe(0);

      const editButtons = await page.locator('[data-testid="faq-edit-button"]').count();
      expect(editButtons).toBe(0);

      const deleteButtons = await page.locator('[data-testid="faq-delete-button"]').count();
      expect(deleteButtons).toBe(0);
    }
  });

  test("not signed in: arrow keys do not trigger API calls (canEdit should be false)", async ({ page }) => {
    const eventPage = newEventPage(page);
    const { faqPage } = eventPage;
    ...
    if (faqCount > 0) {
      ...
      page.on("request", (request) => {
        ...
        if (url.includes("/faq") && (method === "PATCH" || method === "PUT")) {
...
test.describe("Event FAQ Keyboard Permissions - Admin User", { tag: "@desktop" }, () => {
  test.beforeEach(async ({ page }) => {
    await navigateToFirstEvent(page);
  ...
  test("admin user: can focus FAQ cards (canEdit should be true)", async ({ page }) => {
    const eventPage = newEventPage(page);
    const { faqPage } = eventPage;
  ...
  test("admin user: edit controls are visible (canEdit should be true)", async ({ page }) => {
    const eventPage = newEventPage(page);
    const { faqPage } = eventPage;
  ...
  test("admin user: can reorder FAQs with keyboard (canEdit should be true)", async ({ page }) => {
    const eventPage = newEventPage(page);
    const { faqPage } = eventPage;
    ...
    if (faqCount >= 2) {
      const getQuestionTexts = async () => {
        const questions = await page.locator('[data-testid="faq-question"]').allTextContents();
        return questions;
      };
