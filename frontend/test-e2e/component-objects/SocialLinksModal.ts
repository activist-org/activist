...
export const newSocialLinksModal = (parent: Page | Locator) => {
  const root = parent.locator('[role="dialog"]').filter({ hasText: /social.*links/i });
  ...
  return {
    ...
    // Form elements
    labelInputs: root.locator('input[name*="label"], input[placeholder*="label"]'),
    urlInputs: root.locator('input[name*="link"], input[name*="url"], input[placeholder*="url"]'),
    
    // Buttons
    addLinkButton: root.getByRole("button", { name: /add.*link/i }),
    updateButton: root.getByRole("button", { name: /update/i }),
    removeButtons: root.locator('button:has-text("×"), button[aria-label*="remove"]'),
    closeButton: root.locator('button[aria-label*="close"], button:has-text("×")').first(),
    ...
    fillSocialLink: async (index: number, label: string, url: string) => {
      const labelInputs = root.locator('input[name*="label"], input[placeholder*="label"]');
      const urlInputs = root.locator('input[name*="link"], input[name*="url"], input[placeholder*="url"]');
