<template>
  ...
  >
    <Head>
      <Title>{{ event.name }}&nbsp;{{ $t("_global.team_lower") }}</Title>
    </Head>
    <HeaderAppPage
      :event="event"
      :header="event.name + ' ' + $t('_global.team_lower')"
      :tagline="$t('pages.events.team.tagline')"
    ...
    >
      <div class="flex space-x-2 lg:space-x-3">
        <BtnLabeled
          ...
          leftIcon="bi:plus-lg"
          iconSize="1.35em"
          ariaLabel="components.btn-labeled.invite-someone-event-aria-label"
        />
      </div>
    </HeaderAppPage>
  </div>
