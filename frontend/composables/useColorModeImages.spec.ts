...
import useColorModeImages from './useColorModeImages';

mockNuxtImport('useColorMode', () => useColorModeMock);

describe('useColorModeImage', () => {
  it('adds _light to image path', () => {
    useColorModeMock.mockImplementation(() => ({ preference: 'light', value: 'light' }));

    const getImage = useColorModeImages();
    expect(getImage('/path/to/image', '.png')).toEqual('/path/to/image_light.png');
  });
  ...
  it('adds _dark to image path', () => {
    ...
    const getImage = useColorModeImages();
    expect(getImage('/path/to/image', '.png')).toEqual('/path/to/image_dark.png');
  });
