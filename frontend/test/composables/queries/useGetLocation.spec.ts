...
/**
 * Unit tests for useGetLocation composable.
 */
...
describe("useGetLocation", () => {
  ...
  describe("Composable Structure", () => {
    it("returns an object with data property", () => {
      const result = useGetLocation({ city: "Madrid" });
  ...
  describe("Parameter Handling", () => {
    it("accepts a static record object", () => {
      const result = useGetLocation({ city: "Vienna" });
      expect(result).toBeDefined();
    });

    it("accepts null without breaking", () => {
      const result = useGetLocation(null);
      expect(result).toBeDefined();
    });

    it("returns same structure regardless of parameter", () => {
      const result1 = useGetLocation({ city: "A" });
      const result2 = useGetLocation(null);
