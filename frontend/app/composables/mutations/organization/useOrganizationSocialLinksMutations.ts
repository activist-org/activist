...
export function useOrganizationSocialLinksMutations(
  ...
  const {
    ...
    mutation: async (links: SocialLinkInput[]) => {
      if (!currentOrgId.value || !links.length) return null;
      return createOrganizationSocialLinks(currentOrgId.value, links);
    },
    async onSuccess() {
      await invalidateOrganizationCache(currentOrgId.value);
  ...
  const {
    ...
    mutation: async (
      ...
      if (!currentOrgId.value) return null;
      return replaceAllOrganizationSocialLinks(currentOrgId.value, links);
    },
    async onSuccess() {
      await invalidateOrganizationCache(currentOrgId.value);
