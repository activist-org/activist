...
export const getKeyForGetOrganizationsByUser = (userId: string, page:number, filters?: OrganizationFilters) => `organizations-by-user-${userId}:page:${page}:filters:${JSON.stringify(filters)}`;

export function useGetOrganizationsByUser(userId: string, filters?: MaybeRef<OrganizationFilters>) {
  ...
  const { data, pending, error, refresh } = useAsyncData<Organization[]>(
    ...
    async () => {
      try {
        ...
        const paginatedOrganizations = await listOrganizationsByUserId(userIdRef.value, page.value, filtersRef.value);
