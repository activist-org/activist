...
"""
def post_update(test_uuid: UUID, test_group: GroupFactory, test_order: str, test_link: str, test_label:str):
    client = Client()
    response = client.put(
...
def test_update_social_links(client: Client) -> None:
    ...
    bad_uuid = uuid.uuid4()
    response = post_update(test_uuid=bad_uuid, test_group=test_update_group, test_order=test_order, test_link=test_link, test_label=test_label)
    ...
    if login_response.status_code == 200:
        response = post_update(test_uuid=group_id, test_group=test_update_group, test_order=test_order, test_link=test_link, test_label=test_label)
