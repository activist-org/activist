...
class TypeChecker:
    ...
    def __init__(self, models_file: str, types_file: str):
    ...
    def check(self) -> List[str]:
        ...
        for model_name, fields in self.model_fields.items():
            ...
            if not interfaces:
                missing_fields.append(self._format_missing_interface_message(model_name))
                continue
    ...
    def _is_field_accounted_for(self, field: str, interfaces: Dict[str, Set[str]]) -> bool:
        """Check if a field is accounted for in TypeScript."""
        camel_field = snake_to_camel(field)
        return (camel_field in self.backend_only or
                field in self.backend_only or
                any(camel_field in fields for fields in interfaces.values()))
    ...
    def _format_missing_interface_message(model_name: str) -> str:
        ...
        return (f"No matching TypeScript interface found for model: {model_name}\n"
                f"Searched for interfaces: {', '.join(potential_names)}")

    @staticmethod
    def _format_missing_field_message(field: str, model_name: str,
                                    interfaces: Dict[str, Set[str]]) -> str:
        ...
        return (
            ...
            f"To ignore this field, add a comment: '// Note: {camel_field} is backend only'"
        )
